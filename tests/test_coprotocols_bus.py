...
@pytest.mark.asyncio
async def test__threadbased_coprotocol(test_suite: ServerTestSuite):
    agent1_params = test_suite.get_agent_params('agent1')
    agent2_params = test_suite.get_agent_params('agent2')
...
@pytest.mark.asyncio
async def test__coprotocols_intersections(test_suite: ServerTestSuite):
    agent1_params = test_suite.get_agent_params('agent1')
    agent2_params = test_suite.get_agent_params('agent2')
    ...
    thread_id = uuid.uuid4().hex
    co1_threaded = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedP2P(thread_id, pairwise1)
    co2_threaded = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedP2P(thread_id, pairwise2)
    co1_p2p = sirius_sdk.hub.coprotocols_bus.CoProtocolP2P(pairwise1, ['test_protocol'])
    co2_p2p = sirius_sdk.hub.coprotocols_bus.CoProtocolP2P(pairwise2, ['test_protocol'])
    try:
        MSG_LOG.clear()
        await run_coroutines(
            routine1(co1_threaded, **agent1_params), routine2(co2_threaded, **agent2_params),
            routine1(co1_p2p, **agent1_params), routine2(co2_p2p, **agent2_params),
        )
        # collect messages
...
async def test_coprotocol_abort(test_suite: ServerTestSuite, agent1: Agent, agent2: Agent):
    ...
    co = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedP2P(thid=uuid.uuid4().hex, to=pw1, time_to_live=100)
...
async def test_coprotocol_abort_multiple_ops_single_hub(test_suite: ServerTestSuite, agent1: Agent, agent2: Agent):
    ...
    co = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedP2P(thid=uuid.uuid4().hex, to=pw1)
    ...
    async def coroutine1(server_address: str, credentials: bytes, p2p: sirius_sdk.P2PConnection, **kwargs):
        ...
        async with sirius_sdk.context(server_address, credentials, p2p):
            ...
            try:
                new_co_on_same_hub = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedP2P(thid=new_thread_id, to=pw1)
    ...
    async def coroutine2(server_address: str, credentials: bytes, p2p: sirius_sdk.P2PConnection, **kwargs):
        ...
        async with sirius_sdk.context(server_address, credentials, p2p):
            try:
                new_co_on_same_hub = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedP2P(thid=new_thread_id, to=pw2)
...
async def test_coprotocol_threaded_theirs_send(
    ...
    async def sender(server_address: str, credentials: bytes, p2p: sirius_sdk.P2PConnection, **kwargs):
        ...
        async with sirius_sdk.context(server_address, credentials, p2p):
            ...
            co = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedTheirs(thread_id, [pw1, pw2])
            await co.send(msg)
...
async def test_coprotocol_threaded_theirs_switch_timeout(
    ...
    async def actor(server_address: str, credentials: bytes, p2p: sirius_sdk.P2PConnection, **kwargs):
        ...
        async with sirius_sdk.context(server_address, credentials, p2p):
            msg = sirius_sdk.aries_rfc.Ping(comment='Test Ping')
            co = sirius_sdk.hub.coprotocols_bus.CoProtocolThreadedTheirs(thread_id, [pw1, pw2], time_to_live=ttl)
            statuses = await co.switch(msg)
