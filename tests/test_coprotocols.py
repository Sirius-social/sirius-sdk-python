...
from sirius_sdk.encryption.custom import b58_to_bytes, bytes_to_b58
...
async def run_coroutines(*args):
    ...
    done, pending = await asyncio.wait(items, timeout=60, return_when=asyncio.FIRST_EXCEPTION)
    for f in done:
        if f.exception():
            raise f.exception()
    for f in pending:
