...
async def test_same_account(test_suite: ServerTestSuite):
    ...
    try:
        ...
        ok, busy = await session1.locks.acquire(resources=resources, lock_timeout=5)
        try:
            assert ok is True
            ok, busy = await session2.locks.acquire(resources=resources, lock_timeout=1)
            assert ok is False
            assert set(busy) == set(resources)
        finally:
            await session1.locks.release()
        ...
        resources = [f'resource-{uuid.uuid4().hex}' for i in range(100)]
        timeout = 5.0
        ok, _ = await session1.locks.acquire(resources=resources, lock_timeout=timeout)
        assert ok is True
        ok, _ = await session2.locks.acquire(resources=resources, lock_timeout=1.0)
        assert ok is False
...
async def test_lock_multiple_time(test_suite: ServerTestSuite):
    ...
    try:
        ...
        timeout = 5.0
        ok, _ = await session1.locks.acquire(resources=resources1, lock_timeout=timeout)
        assert ok is True

        resources2 = [f'resource-{uuid.uuid4().hex}']
        ok, _ = await session1.locks.acquire(resources=resources2, lock_timeout=timeout)
        assert ok is True
        # session1 must unlock previously locked resources on new acquire call
        ok, _ = await session2.locks.acquire(resources=resources1, lock_timeout=timeout)
        assert ok is True
    finally:
        await session1.close()
        await session2.close()
...
async def test_different_accounts(test_suite: ServerTestSuite):
    ...
    try:
        ...
        ok1, _ = await agent1.locks.acquire(resources=same_resources, lock_timeout=10.0)
        ok2, _ = await agent1.locks.acquire(resources=same_resources, lock_timeout=10.0)
